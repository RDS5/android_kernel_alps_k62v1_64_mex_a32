 ...
 * Description: This file implements communication between kernel and user space.
...
/**
 * function_name() - notify_chr_thread_to_send_msg
	...
	unsigned int src_addr, struct sock *sk)
{
	...
	if (g_chr_module_state == CHR_NETLINK_INIT) {
		...
		/* save server's address and port when timer state is CHR_TIMER_3 */
...
/**
 * function_name() - kernel_chr_receive
 * @arg1: socket buffer
 *
...
static void update_net_iface_dst_addr(
	struct chr_net_iface_struct *net_if_ptr)
{
	int j;
...
/**
 * function_name() - chr_netlink_thread
 * @arg1: data stream
 *
