...
static unsigned int g_use_two_in_one_speaker = 0; // 1:use,0:not use
...
#if defined(CONFIG_SND_SOC_MTK_AUDIO_PA)
	AudDrv_GPIO_EXTAMP_Select((int)enable,
		mCodec_data->mAudio_Ana_DevicePower
...
static int audio_switch_mode_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = mCodec_data->mAudio_Ana_DevicePower
					[AUDIO_ANALOG_DEVICE_SWITCH_MODE];
	return 0;
}

static int audio_switch_mode_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s(), mode %ld", __func__, ucontrol->value.integer.value[0]);
	mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_SWITCH_MODE] =
		ucontrol->value.integer.value[0];

	return 0;
}

static void ext_speaker_component_apply(int component, bool enable)
{
	switch (component) {
	case COMPONENT_SWITCH:
		auddrv_gpio_select_by_switch(mCodec_data->mAudio_Ana_DevicePower
					[AUDIO_ANALOG_DEVICE_SWITCH_MODE]);
		break;
	case COMPONENT_PA:
		Ext_Speaker_Amp_Change(enable);
		break;
	default:
		pr_err("%s, input component %d error", __func__, component);
		break;
	}
}

static void ext_speaker_component_change(bool enable)
{
	int i;
	int component[COMPONENT_NUM] = { COMPONENT_SWITCH, COMPONENT_PA };
	static int pre_pa_mode = 0;

	// If the mode changes from large to small, COMPONENT_PA is set first
	if ((pre_pa_mode != 0) &&
		(pre_pa_mode < mCodec_data->mAudio_Ana_DevicePower
					[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP])) {
		component[COMPONENT_SWITCH] = COMPONENT_PA;
		component[COMPONENT_PA] = COMPONENT_SWITCH;
	}

	pre_pa_mode = mCodec_data->mAudio_Ana_DevicePower
					[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP];

	for (i = 0; i < ARRAY_SIZE(component); i++) {
		udelay(800);
		ext_speaker_component_apply(component[i], enable);
	}
}
...
static int Ext_Speaker_Amp_Set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s(), gain %ld", __func__, ucontrol->value.integer.value[0]);

	mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP] =
		ucontrol->value.integer.value[0];

	if (ucontrol->value.integer.value[0]) {
		if (g_use_two_in_one_speaker == 1) // use two-in-one speaker
			ext_speaker_component_change(true);
		else
			Ext_Speaker_Amp_Change(true);
	} else {
		if (g_use_two_in_one_speaker == 1) // use two-in-one speaker
			ext_speaker_component_change(false);
		else
...
static const char *const switch_mode_function[] = { "0", "12", "24" };
...
static const struct soc_enum Audio_DL_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(apply_n12db_setting),
			    apply_n12db_setting),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(switch_mode_function),
...
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
...
#if defined(CONFIG_SND_SOC_MTK_AUDIO_PA)
	SOC_ENUM_EXT("Ext_Speaker_Amp_Switch", Audio_DL_Enum[16],
#else
	...
	SOC_ENUM_EXT("Audio_Switch_Mode", Audio_DL_Enum[15],
		audio_switch_mode_get, audio_switch_mode_set),
};
...
static int mtk_mt6357_codec_dev_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		(void)of_property_read_u32(pdev->dev.of_node,
			"use_two_in_one_speaker", &g_use_two_in_one_speaker);
		pr_info("%s(), use_two_in_one_speaker %u",
			__func__, g_use_two_in_one_speaker);
	} else {
