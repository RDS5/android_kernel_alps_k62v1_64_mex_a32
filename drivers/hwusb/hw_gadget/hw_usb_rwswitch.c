...
static ssize_t switch_request_show(struct device *pdev,
...
static ssize_t switch_request_store(struct device *pdev,
	struct device_attribute *attr, const char *buff, size_t size)
{
	char event_buf[EVENT_BUF_LEN];
	char *envp[2] = {event_buf, NULL};

	snprintf(event_buf, sizeof(event_buf), "USB_PORT_SWITCH_DONE=%d",
		switch_request);
...
DEVICE_ATTR(switch_request, 0644, switch_request_show,
	switch_request_store);
