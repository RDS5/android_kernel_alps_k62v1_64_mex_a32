...
#define COPY_TASK_COMM(p, q)	strncpy(p, q, TASK_COMM_LEN - 1)
#define COPY_PROC_NAME(p, q)	strncpy(p, q, NAME_LEN - 1)
...
int dubai_get_proc_name(void __user *argp)
{
	...
	size = sizeof(struct dev_transmit_t) + sizeof(struct process_name);
	transmit = kzalloc(size, GFP_KERNEL);
	if (transmit == NULL) {
