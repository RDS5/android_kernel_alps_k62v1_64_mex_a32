...
static void battery_temp_table_parse(struct device_node *np)
{
	int num = 0;
	unsigned int idx;
	int temp = 0;
	...
	ret = of_property_read_u32(np, "batt_temperature_table_num", &num);
	if (ret || num != FG_TEMPERATURE_TABLE_SIZE) {
		bm_err("can not read custom batt temp table\n");
		return;
	}

	for (idx = 0; idx < num; idx++) {
		/* idx * 2 : temp in every row of table */
		if (of_property_read_u32_index(np, "batt_temperature_table",
				(idx * 2), &temp)) {
			bm_err("read idx: %d temp failed\n", idx);
			break;
		}
		/* idx * 2 + 1 : temp_r in every row of table */
		if (of_property_read_u32_index(np, "batt_temperature_table",
				(idx * 2 + 1), &temp_r)) {
			bm_err("read idx: %d temp_r failed\n", idx);
			break;
		}
		bm_info("Fg_Temperature_Table: %d, %d\n", temp, temp_r);
		Fg_Temperature_Table[idx].BatteryTemp = temp;
		Fg_Temperature_Table[idx].TemperatureR = temp_r;
	}
}
