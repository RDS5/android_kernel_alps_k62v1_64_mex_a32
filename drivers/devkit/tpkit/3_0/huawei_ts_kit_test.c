...
int test_dbg_cmd_test(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	case TEST_CMDQ_ITEM_3:
		if (!cq_test_buff_index) {
			...
			wait_for_completion(&cq_test_sync_1);
			smp_mb();
		}
		...
		cq_test_buff_index++;
		if (cq_test_buff[cq_test_buff_index] == -1) {
			/* whether reach the end of buffer */
...
int cq_item_1_func(void *data)
{
	...
	for (i = 0; i < 100; i++) {
		...
		if (ts_kit_put_one_cmd(&cmd) == NO_ERR) {
...
int cq_item_3_func(void *data)
{
	...
	for (i = 0; i < TS_CMD_QUEUE_SIZE + 5; i++) {
		...
		if (ts_kit_put_one_cmd(&cmd)) {
			...
			/* finish 1 test break form internal loop */
			goto out;
		}
	}
...
static int test_create_debugfs(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_test_items); i++) {
		dent = debugfs_create_u32(g_test_items[i].item_name,
			S_IRUGO | S_IWUSR, dir,
