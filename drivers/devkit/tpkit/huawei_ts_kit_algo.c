...
static int ts_detect_ghost_algo1(int index)
{
	...
	if ((delta_time > 0) && (delta_time < FINGER_PRESS_TIME_MIN_ALGO1)) {
		TS_LOG_DEBUG(
			"%s:%s suspiciou ghost press "
			"finger_release_tv.tv_sec:%lu, "
			"finger_release_tv.tv_usec:%lu, "
			"finger_press_tv[%d].tv_sec:%lu, "
			"finger_press_tv[%d].tv_usec:%lu, "
			"delta time:%lu\n",
			__func__, GHOST_LOG_TAG,
			finger_touch.finger_release_tv.tv_sec,
			finger_touch.finger_release_tv.tv_usec,
			index, finger_touch.finger_press_tv[index].tv_sec,
			index, finger_touch.finger_press_tv[index].tv_usec,
			delta_time);
		...
		if (ghost_num_record_per_second[index] >
			FINGER_PRESS_TIMES_IN_MIN_TIME_ALGO1) {
			ghost_num_record_per_second[index] = 0;
			ghost_time_record[index] = 0;
...
/*
 * If there are FINGER_PRESS_TIMES_IN_MIN_TIME_ALGO2
...
static int ts_detect_ghost_algo3(int finger_num)
{
	...
	if ((finger_num == GHOST_OPERATE_TWO_FINGERS) &&
		...
		if (target_x == 0) {
			...
			if (ghost_algo3_num == FINGER_PRESS_TIMES_IN_MIN_TIME_ALGO3) {
				TS_LOG_INFO("%s:%s DETECT:%d\n", __func__,
	...
	struct ts_fingers *in_info, struct ts_fingers *out_info)
{
	...
	if (in_info->cur_finger_number == 0) {
			...
			if (in_info->fingers[index].status != 0) {
				...
				if (must_report_flag & (1 << index)) {
					...
					if (stop_report_flag & (1 << index)) {
						...
						if (temp_val) {
							TS_LOG_DEBUG(
								"stopped finger index: %u, coordinate is legal1, "
								"can report again\n", index);
		...
		TS_LOG_DEBUG(
			"1_must_report_flag=%d, stop=%d, temp=%d, L_RES_ADDR=%d,"
			"L_RES_ERT=%d, R_RES_ADDR=%d, R_RES_ERT=%d\n",
		...
		TS_LOG_DEBUG(
			"2_must_report_flag=%d, stop=%d, temp=%d, L_RES_ADDR=%d, "
			"L_RES_ERT=%d, R_RES_ADDR=%d, R_RES_ERT=%d\n",
			must_report_flag, stop_report_flag, temp_report_flag,
			left_res_point[RES_ADDR], left_res_point[RES_ERT],
			right_res_point[RES_ADDR], right_res_point[RES_ERT]);
