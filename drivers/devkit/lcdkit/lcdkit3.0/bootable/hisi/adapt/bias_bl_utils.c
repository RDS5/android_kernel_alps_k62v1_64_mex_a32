	...
	const char *prop_name, int *data, unsigned int *len)
{
	...
	if (fdt_ops == NULL) {
		FASTBOOT_ERR("can't get fdt operators\n");
		return LCD_RET_FAIL;
	}
	dtimage_ops = get_operators(DTIMAGE_MODULE_NAME_STR);
	if (dtimage_ops == NULL ||
		fdt_ops->fdt_node_offset_by_compatible == NULL ||
		fdt_ops->fdt_operate_lock == NULL ||
		dtimage_ops->get_dtb_addr == NULL ||
		fdt_ops->fdt_get_property == NULL ||
		fdt_ops->fdt_operate_unlock == NULL) {
...
static void dts_set_ic_name(const char *prop_name, const char *name)
{
	...
	dtimage_ops = get_operators(DTIMAGE_MODULE_NAME_STR);
	if (dtimage_ops == NULL) {
		FASTBOOT_ERR("failed to get dtimage operators!\n");
		return;
	}

	if (fdt_ops->fdt_open_into == NULL ||
		fdt_ops->fdt_operate_lock == NULL ||
		dtimage_ops->get_dtb_addr == NULL ||
		fdt_ops->fdt_path_offset == NULL ||
		fdt_ops->fdt_setprop_string == NULL ||
		fdt_ops->fdt_operate_unlock == NULL) {
		FASTBOOT_ERR("ops function is NULL\n");
		return;
	}
...
static int dts_get_ic_num(const char *prop_name)
{
	...
	if (dtimage_ops->getU32byCompatible == NULL) {
		FASTBOOT_ERR("ops function is NULL\n");
		return num;
	}

	ret = dtimage_ops->getU32byCompatible(DTS_COMP_LCD_PANEL_TYPE,
		prop_name, &num);
	if (ret < 0) {
...
static int dts_set_status_by_compatible(const char *name, const char *status)
{
	...
	if (fdt_ops->fdt_node_offset_by_compatible == NULL ||
		fdt_ops->fdt_operate_lock == NULL ||
		dtimage_ops->get_dtb_addr == NULL ||
		fdt_ops->fdt_open_into == NULL ||
		fdt_ops->fdt_setprop_string == NULL ||
		fdt_ops->fdt_operate_unlock == NULL) {
		FASTBOOT_ERR("ops function is NULL\n");
		return LCD_RET_FAIL;
