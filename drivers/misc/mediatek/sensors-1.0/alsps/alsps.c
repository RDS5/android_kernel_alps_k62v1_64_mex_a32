...
int als_data_report_t(int value, int ch0, int ch1, int status, int64_t time_stamp)
{
	int err = 0;
...
int als_data_report(int value, int ch0, int ch1, int status)
{
	return als_data_report_t(value, ch0, ch1, status, 0);
}
...
int ps_data_report_t(int value, int pdata, int raw, int status, int64_t time_stamp)
{
	int err = 0;
	struct sensor_event event;
	...
	pr_notice("[ALS/PS]ps_data_report-%d, pdata-%d\n", value, pdata);
	event.flush_action = DATA_ACTION;
	event.time_stamp = time_stamp;
	event.word[0] = value + 1;
	event.word[1] = pdata;
...
int ps_data_report(int value, int pdata, int raw, int status)
{
	return ps_data_report_t(value, pdata, raw, status, 0);
}
...
static void als_work_func(struct work_struct *work)
{
	...
	als_data_report(cxt->drv_data.als_data.values[0],
		cxt->drv_data.als_data.values[1],
		cxt->drv_data.als_data.values[2],
