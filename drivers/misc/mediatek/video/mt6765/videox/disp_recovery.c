...
extern void lcd_kit_set_backlight_esd_recovery(unsigned int level);
#endif
...
#ifdef CONFIG_LCD_KIT_DRIVER
static unsigned int need_do_esd_bl_recovery(void)
{
	int ret = 0;

	if((primary_get_lcm() == NULL) || (primary_get_lcm()->params == NULL)) {
		DISPCHECK("lcm info is NULL\n");
		return 0;
	}
#ifdef CONFIG_OF
	if ((primary_get_lcm()->params->dsi.esd_recovery_bl_enable == 1) &&
		(islcmconnected == 1))
		ret = 1;
#else
	if (primary_get_lcm()->params->dsi.esd_recovery_bl_enable == 1)
...
#ifdef CONFIG_LCD_KIT_DRIVER
	if (need_do_esd_bl_recovery() && (panel_info->bl_ic_ctrl_mode == MTK_MIPI_BL_IC_PWM_MODE))
		lcd_kit_set_backlight_esd_recovery(esd_recovery_level);
#endif
