...
MUINT32 imgsensor_con_sensor_id(MUINT32 imgsensor_sensor_id,
		MUINT32 sensor_chip_id)
{
	MUINT32 sensor_id;
	kal_uint16 mono_flag;

	imgsensor_sensor_i2c_write(&imgsensor, 0xfe, 0x00,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xfe, 0x00,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xfe, 0x00,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xf7, 0x01,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xf8, 0x0c,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xf9, 0x42,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xfa, 0x88,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xfc, 0x9e,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xd4, 0x80,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xfe, 0x00,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xd5, 0x00,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xf3, 0x20,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_read(&imgsensor, 0xd7, &mono_flag,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xd4, 0x00,
		IMGSENSOR_I2C_BYTE_DATA);
	imgsensor_sensor_i2c_write(&imgsensor, 0xfc, 0x8e,
		IMGSENSOR_I2C_BYTE_DATA);
	LOG_INF("mono_flag: 0x%x", mono_flag);
	// 10:GC2375B-WC1X0, 00:GC2375H-WC1X0, 01:GC2375H-WC1XH
	if ((mono_flag & 0x3) == 0x2) {
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = imgsensor_con_sensor_id(
			...
			LOG_INF("Check sensor id, addr: 0x%x,read id: 0x%x, \
				expect id:0x%x.\n",
				imgsensor.i2c_write_id,
