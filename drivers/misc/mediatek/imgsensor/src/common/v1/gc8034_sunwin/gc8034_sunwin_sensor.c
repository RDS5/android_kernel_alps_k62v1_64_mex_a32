/*
 * gc8034_sunwin_sensor.c
 *
 * Copyright (c) 2020-2020 Huawei Technologies Co., Ltd.
 *
 * gc8034_sunwin image sensor driver
 *
...
#include <linux/i2c.h>
#include "gc8034_sunwin_sensor.h"

#define RETRY_TIMES	2
#define REG_GAIN_OFFSET_LEFT  4


#define PFX "[gc8034_sunwin]"
...
static struct gc8034_sunwin_otp_t otp_info;
#ifdef GC8034_SUNWIN_OTP_FOR_CUSTOMER
...
static void gc8034_sunwin_gcore_read_otp_info(void)
{
...
static void gc8034_sunwin_gcore_check_prsel(void)
{
...
static void gc8034_sunwin_gcore_update_dd(void)
{
	kal_uint8 state = 0;
...
#ifdef GC8034_SUNWIN_OTP_FOR_CUSTOMER
static int gc8034_sunwin_gcore_update_wb(void)
{
...
static void gc8034_sunwin_gcore_update_lsc(void)
{
	kal_uint8 state = 0;
...
static void gc8034_sunwin_gcore_update_chipversion(void)
{
	kal_uint8 i = 0;
...
static void gc8034_sunwin_gcore_update_otp(void)
{
	gc8034_sunwin_gcore_update_dd();
	gc8034_sunwin_gcore_check_prsel();
#ifdef GC8034_SUNWIN_OTP_FOR_CUSTOMER
	gc8034_sunwin_gcore_update_wb();
	gc8034_sunwin_gcore_update_lsc();
#endif
	gc8034_sunwin_gcore_update_chipversion();
}

static void gc8034_sunwin_gcore_enable_otp(kal_uint8 state)
{
...
static void gc8034_sunwin_gcore_initial_otp(void)
{
...
static void gc8034_sunwin_gcore_identify_otp(void)
{
	gc8034_sunwin_gcore_enable_otp(OTP_OPEN);
	gc8034_sunwin_gcore_read_otp_info();
	gc8034_sunwin_gcore_update_otp();
	gc8034_sunwin_gcore_enable_otp(OTP_CLOSE);
}
...
static kal_uint32 return_sensor_id(void)
{
	...
	if (sensor_id == GC8034_SENSOR_ID) {
		gc8034_sunwin_gcore_initial_otp();
		gc8034_sunwin_gcore_enable_otp(OTP_OPEN);
		...
		log_inf("mode_id2 = %d\n", mode_id2);
		gc8034_sunwin_gcore_enable_otp(OTP_CLOSE);
		if (mode_id1 == SANYINGXING_MOD_ID || mode_id2 == SANYINGXING_MOD_ID)
			sensor_id = GC8034_SUNWIN_SENSOR_ID;
		else
			sensor_id = 0;
	}
	return sensor_id;
...
static kal_uint32 open(void)
{
	...
	gc8034_sunwin_gcore_identify_otp();
...
static void gc8034_sunwin_get_crop_info(UINT32 feature,
...
static void gc8034_sunwin_get_mipi_pixel_rate(
...
static void gc8034_sunwin_get_pixel_rate(unsigned long long *feature_data)
{
	switch (*feature_data) {
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
...
static kal_uint32 feature_control_gc8034_sunwin(
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		gc8034_sunwin_get_crop_info(*feature_data_32, wininfo);
		break;
	...
	case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
		gc8034_sunwin_get_mipi_pixel_rate(feature_data);
		break;
	case SENSOR_FEATURE_GET_PIXEL_RATE:
		gc8034_sunwin_get_pixel_rate(feature_data);
		break;
	default:
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	get_info,
	get_resolution,
	feature_control_gc8034_sunwin,
	control,
	close
};

UINT32 GC8034_SUNWIN_SensorInit(struct SENSOR_FUNCTION_STRUCT **pf_func)
{
	if (pf_func)
		*pf_func = &sensor_func;
	return ERROR_NONE;
