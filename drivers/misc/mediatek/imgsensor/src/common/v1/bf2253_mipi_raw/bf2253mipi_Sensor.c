...
#include "bf2253mipi_Sensor.h"

#define PFX "bf2253_camera_sensor"
#define LOG_1 log_inf("BF2253, MIPI 1LANE\n")

#define log_err(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)
#define log_info(format, args...) pr_info(PFX "[%s] " format, __func__, ##args)
#define log_inf(format, args...) pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool bf2253duringtestpattern = KAL_FALSE;
#define DATA_LINE_LENGTH 1236
#define MAX_VB_LINE_LENGTH 8191
#define MIN_VB_LINE_LENGTH 16

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = BF2253MIPI_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
		0000, 0000, 1600, 1200, 0, 0, 1600, 1200}, /* HS video */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	log_inf("%s: isp gain=%d, sensor gain=0x%x\n", __func__, igain, ireg);
...
{
	log_inf(">> %s()\n", __func__);
	if (read_cmos_sensor(0xfb) == 0x01) { // 0x01: version number
		log_info("bf2253 version 1.0");
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		/* if Sensor ID is not correct, set id to 0xFFFFFFFF */
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	bf2253duringtestpattern = KAL_FALSE;
	return ERROR_NONE;
}
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			log_inf("%d fps is not support,use cap's:%d fps!\n",
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps
			...
			if (imgsensor.current_fps
				...
				log_inf("%d fps is not support, use %d fps!\n",
					framerate,
					imgsensor_info.cap.max_framerate / 10);
