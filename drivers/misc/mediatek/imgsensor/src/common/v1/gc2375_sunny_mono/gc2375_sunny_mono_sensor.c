/*
 * gc2375_sunny_mono_sensor.c
 *
 * gc2375_sunny_mono image sensor driver
 *
...
#include "gc2375_sunny_mono_sensor.h"
...
#define PFX "[gc2375_sunny_mono]"
#define DEBUG_GC2375_SUNNY_MONO 1
#define LOG_DBG(fmt, args...) \
	do { \
		if (DEBUG_GC2375_SUNNY_MONO) \
			pr_debug(PFX "%s %d " \
...
static void set_dummy(void)
{
	...
	rc = imgsensor_sensor_i2c_write(&imgsensor, GC2375_PAGE_SEL_REG, 0x00,
                                        IMGSENSOR_I2C_BYTE_DATA);
	if (rc < 0) {
		LOG_ERR("wtire sensor page ctrl reg failed.");
	}
	rc = imgsensor_sensor_i2c_write(&imgsensor, GC2375_VB_REG_H, (vb >> 8) & 0x1f,
		IMGSENSOR_I2C_BYTE_DATA);
	if (rc < 0) {
		LOG_ERR("wtire sensor csictrl capt vb  h reg  failed.");
	}
	rc = imgsensor_sensor_i2c_write(&imgsensor, GC2375_VB_REG_L, vb & 0xff,
		IMGSENSOR_I2C_BYTE_DATA);
	if (rc < 0) {
...
#define QVALUE 64
static kal_int16 gain_to_reg(kal_uint16 gain, gc2375_gain_table_t *gain_table, kal_uint16 size)
{
	kal_int16 i;
	kal_int16 real_gain;
...
MUINT32 imgsensor_mono_sensor_id(MUINT32 imgsensor_sensor_id, MUINT32 sensor_chip_id)
{
	MUINT32 sensor_id;
	MUINT32 imgsensor_id;
	kal_uint16 mono_flag;
	...
	LOG_INF("gc2375_mono mono_flag: 0x%x", mono_flag);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = imgsensor_mono_sensor_id(
...
static kal_uint32 feature_control_gc2375_sunny_mono(MSDK_SENSOR_FEATURE_ENUM feature_id,
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	get_info,
	get_resolution,
	feature_control_gc2375_sunny_mono,
	control,
	close
};

UINT32 GC2375_SUNNY_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pf_func)
{
