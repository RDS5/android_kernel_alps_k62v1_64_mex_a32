...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		if (strncmp(CONFIG_ARCH_MTK_PROJECT, "tb8768p1", 8) == 0) {
			...
			extern int camcal_read_insensor_gc5035p_qunhui_func(unsigned char *eeprom_buff,
							int eeprom_size);
			...
			} else if (ptempbuf->sensorID == GC5035P_MIPI_SENSOR_ID) {
			...
			} else if (ptempbuf->sensorID == GC5035P_QUNHUI_MIPI_SENSOR_ID) {
				i4RetValue = camcal_read_insensor_gc5035p_qunhui_func(pu1Params, ptempbuf->u4Length);
				if (i4RetValue < 0) {
					pr_err("gc5035 qunhui read otp data error!\n");
					kfree(pBuff);
					kfree(pu1Params);
					return -EFAULT;
				} else {
