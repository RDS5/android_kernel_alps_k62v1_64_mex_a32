...
static int hps_cal_cores(void)
{
	...
	rush_boost_cores = 0;
	if ((hps_ctxt.cur_loads > hps_ctxt.rush_boost_threshold *
		hps_sys.total_online_cores) &&
		(hps_sys.total_online_cores * 100 < hps_ctxt.cur_tlp)) {
		rush_boost_cores = hps_ctxt.cur_tlp / 100 +
		(hps_ctxt.cur_tlp % 100 ? 1 : 0);
	}
	WARN_ON(hps_ctxt.up_threshold == 0);
	sys_cores = hps_ctxt.cur_loads / hps_ctxt.up_threshold +
		...
		if (hps_ctxt.up_loads_count >= hps_ctxt.up_times) {
			if (hps_ctxt.up_loads_sum >
				...
				if (val > base_val)
					...
					val = 0;
				hps_sys.up_load_avg = hps_ctxt.up_loads_sum /
					hps_ctxt.up_times;
				hps_cal_core_num(&hps_sys, val, base_val);

				return 1;
			}
		}		/* if (hps_ctxt.up_loads_count */
				/*>= hps_ctxt.up_times) */
	}
	return 0;
}
	...
	if (hps_sys.total_online_cores > 1) {
		...
		if (hps_ctxt.down_loads_count >= hps_ctxt.down_times) {
			unsigned int down_threshold =
				hps_ctxt.down_threshold * hps_ctxt.down_times;

			val = hps_sys.total_online_cores;

			while (hps_ctxt.down_loads_sum < down_threshold *
			(val - 1))
				val--;
			WARN_ON(val < 0);
			if (val > base_val)
				...
				val = 0;
			hps_sys.down_load_avg = hps_ctxt.down_loads_sum /
				hps_ctxt.down_times;
			hps_cal_core_num(&hps_sys, val, base_val);
