...
/* build-in fm gpio port: expected 159, otherwise -1 */
...
static unsigned int build_in_gpio = 0;
...
void setHeadsetStatus(bool status)
{
	...
	if (fm_node && is_headset_on && gpio_is_valid(build_in_gpio)) {
		WCN_DBG(FM_NTC | MAIN, "%s: [fm build-in] fm_node_Headset is plug-in, pull down gpio\n", __func__);
		gpio_set_value_cansleep(build_in_gpio, GPIO_PULL_DOWN);
	}
...
{
	int ret = 0;
	build_in_gpio = of_get_named_gpio_flags(fm_node, "fm,fm_elna_pins", 0,NULL);
	ret = gpio_request(build_in_gpio, "fm_elna_pins");
	WCN_DBG(FM_NTC | MAIN,"%s: [fm build-in] request gpio: ret=%d,gpio=%d\n", __func__, ret,build_in_gpio);

	if (ret) {
		build_in_gpio = DEFAULT_FM_LNA_NONEED;
		WCN_DBG(FM_ERR | MAIN,"%s: [fm build-in] Failed to request gpio %d!\n", __func__, build_in_gpio);
		goto err;
	}

	if (gpio_direction_output(build_in_gpio, GPIO_PULL_DOWN)) {
		WCN_DBG(FM_ERR | MAIN,"%s: [fm build-in] gpio set output failed!\n", __func__);
		gpio_free(build_in_gpio);
		build_in_gpio = DEFAULT_FM_LNA_NONEED;
		goto err;
	}
	return true;
...
static long fm_ops_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FM_IOCTL_POWERUP:{
			...
			if (fm_node) {
				if ((!is_headset_on) && gpio_is_valid(build_in_gpio)) {
					WCN_DBG(FM_NTC | MAIN, "pull up gpio\n");
					gpio_set_value_cansleep(build_in_gpio, GPIO_PULL_UP);
				}
				else {
				...
				if ((!is_headset_on) && gpio_is_valid(g_lna_en_gpio)) {
					...
					WCN_DBG(FM_ERR | MAIN, "headset in or gpio is unvalid\n");
				}
			}
	...
	case FM_IOCTL_POWERDOWN:{
			...
			if (fm_node) {
				if (gpio_is_valid(build_in_gpio)) {
...
static signed int mt_fm_probe(struct platform_device *pdev)
{
	...
	if (get_fm_dtsi()) {
		if (true == init_dtsi_lna()) {
			WCN_DBG(FM_NTC | MAIN, "init dtsi lna way success\n");
		}
		else {
			WCN_DBG(FM_NTC | MAIN, "init dtsi lna way failed\n");
		}
	}
	else {
		if (true == init_lna()) {
			WCN_DBG(FM_NTC | MAIN, "init lna success\n");
		}
		else {
			WCN_DBG(FM_ERR | MAIN, "init lna failed\n");
		}
	}
	return ret;
}
