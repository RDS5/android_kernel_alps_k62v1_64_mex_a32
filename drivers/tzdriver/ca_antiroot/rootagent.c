...
DEFINE_MUTEX(eima_ca_lock);

#define PM_SUSPEND  0x1 /*1 for suspend*/
#define PM_RESUME 0x0 /*0 for resume successful*/
int antiroot_sr_flag = PM_RESUME;

#ifdef CONFIG_HUAWEI_ROOT_SCAN_DUMMY_API
static uint32_t antiroot_loglevel_mask = 255;
#else
static uint32_t antiroot_loglevel_mask;
#endif

uint32_t get_antiroot_loglevel_mask(void)
{
	return antiroot_loglevel_mask;
}

#ifdef DEBUG_LOGMASK
module_param_named(loglevel_mask, antiroot_loglevel_mask, uint, S_IWUSR | S_IRUGO);
#endif
...
static int send_response(const struct RAGENT_CHALLENGE *chal)
{
	...
	if (0 != op_mask)
		ret_raw = rscan_dynamic_raw_and_upload(op_mask, gscan);
	...
	for (id_count = 0; id_count < CHALLENGE_MAX_LENGTH &&
		...
		case RAOPID_SELINUX_HOOK:
			...
			break;
		case RAOPID_SETID:
			if (ret_raw & D_RSOPID_SETID)
				resp.runtime_white_list.setid = gconfig.white_list.setid;
			else
				resp.runtime_white_list.setid = gscan->setid;
			break;
		case RAOPID_NOOP:
			resp.noop = 0;
			break;
...
static int tee_init(void)
{
	...
	gconfig.white_list.setid = gscan->setid;
...
{
	if (0 == queue_work(system_wq, &module_init_work))
		antiroot_error("ca add system work queue failed\n");
