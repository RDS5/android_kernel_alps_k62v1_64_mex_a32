...
#ifndef CONFIG_ARM64
#define SECUREC_SPECIAL_64(_val, Base)                                 \
    case Base:                                              \
    do {                                                    \
        *--text.sz = digits[do_div(_val, Base)];                   \
    }while (_val = div_u64(_val, Base) /* != 0 */);                     \
    break
...
#else
							SECUREC_SPECIAL_64(number, 10);
							SECUREC_SPECIAL_64(number, 16);
							SECUREC_SPECIAL_64(number, 8);
#endif
						}
