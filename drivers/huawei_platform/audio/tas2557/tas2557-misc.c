...
static ssize_t tas2557_file_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
	...
	unsigned int reg = 0;
	unsigned int len = 0;
    int low_temp_state = LOW_TEMP_STATE_DEFAULT;

	mutex_lock(&pTAS2557->file_lock);

	if (count == 0) {
		dev_err(pTAS2557->dev, "count == 0\n");
		goto err;
	}
	...
	case TIAUDIO_CMD_CONFIGURATION:
	{
		if (count == 2) {
			...
			&& (pTAS2557->mpFirmware->mnPrograms > 0)) {
				...
				if (low_temp_state == LOW_TEMP_STATE_IN_STAGE2 &&
						(p_kBuf[1] == TAS2557_CONFIG_RINGTONE ||
						p_kBuf[1] == TAS2557_CONFIG_RINGTONE_HS_SPK)) {
					tas2557_set_config(pTAS2557,
	...
	case TIAUDIO_CMD_SPEAKER:
		if (count == 2) {
			low_temp_state = low_temperature_state_get();
			if (low_temp_state == LOW_TEMP_STATE_IN_STAGE1 ||
				low_temp_state == LOW_TEMP_STATE_IN_STAGE2)
				tas2557_agc_enable(pTAS2557, true);
			else
				tas2557_agc_enable(pTAS2557, false);
	...
	case TIAUDIO_CMD_AGC:
		if (count == 2) {
			if (g_logEnable)
                                dev_info(pTAS2557->dev, "TIAUDIO_CMD_AGC, set to %d\n", p_kBuf[1]);
			tas2557_agc_enable(pTAS2557, (p_kBuf[1] > 0));
		}
	break;
...
#ifdef CONFIG_PM
static int tas2557_misc_suspend(void)
{
	struct tas2557_priv *pTAS2557 = g_tas2557;
	int ret = 0;
...
static int tas2557_misc_resume(void)
{
	struct tas2557_priv *pTAS2557 = g_tas2557;
	int ret = 0;
...
#ifdef CONFIG_PM
		ret = register_pm_notifier(&pm_notifier_func);
		if (ret) {
			     dev_err(pTAS2557->dev,"Failed to register PM notifier.\n");
		}
#endif /* CONFIG_PM */
