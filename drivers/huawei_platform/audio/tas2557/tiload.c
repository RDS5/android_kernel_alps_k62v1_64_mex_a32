...
{
	struct tas2557_priv *pTAS2557 = (struct tas2557_priv *)filp->private_data;
	static char rd_data[MAX_LENGTH + 1] = {'\0'};
	...
	size_t count, loff_t *offset)
{
	...
	if (count > MAX_LENGTH || count < MIN_LENGTH) {
		dev_err(pTAS2557->dev, "Max or min %zu bytes can not  be write\n", count);
		return -EINVAL;
	}
