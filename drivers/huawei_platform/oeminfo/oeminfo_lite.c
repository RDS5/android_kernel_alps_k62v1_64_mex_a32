...
{
	if (*index <= 0 || *index >= ARRAY_SIZE(g_oeminfoLiteTable))
		return 0;
...
static enum oeminfo_region_valid_t get_valid_oeminfo_region(int index)
{
	...
	enum oeminfo_region_valid_t valid_region = OEMINFO_REGION_ERROR;
...
static int set_oeminfo(int index, int len, char *data)
{
	unsigned int offset;
	int ret;
	unsigned char oeminfo_hdr_page_buff[OEMINFO_PAGE_SIZE] = {0};
...
/* export oeminfo_direct_access, so we can use it in other procedures */
