...
/**
    @function: unsigned int bfmr_capture_log_from_system(char *buf, unsigned int buf_len, struct bfmr_log_src *src, int timeout)
    ...
    @note:
*/
unsigned int bfmr_capture_log_from_system(char *buf, unsigned int buf_len, struct bfmr_log_src *src, int timeout)
{
...
{
    int ret = 0;
    bfmr_catch_save_bf_log capture_and_save_bootfail_log_tmp;
    ...
    capture_and_save_bootfail_log_tmp = (bfmr_catch_save_bf_log)(param->catch_save_bf_log);
...
/**
    @function: int bfm_platform_process_boot_fail(struct bfm_param *param)
...
{
    int ret;
    struct bfm_record_Info *brit = bfmr_malloc(sizeof(struct bfm_record_Info));
    ...
    ret = bfmr_read_log_from_raw_part(BFM_QCOM_RAW_PART_NAME,
                              ...
                              brit, sizeof(struct bfm_record_Info));
    if (ret)
    {
      ret = -1;
      goto __out;
...
void bfmr_alloc_and_init_raw_log_info(struct bfm_param *pparam, struct bfmr_log_dst *pdst)
{
    struct bfm_record_Info *brit;
...
void bfmr_update_raw_log_info(struct bfmr_log_src *psrc, struct bfmr_log_dst *pdst, unsigned int bytes_read)
{
    struct bfm_record_Info *brit;
