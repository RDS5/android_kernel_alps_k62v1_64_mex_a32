...
extern int iocache_drains_notify(struct notifier_block *self,unsigned long event, void *data);
#endif
...
#ifdef CONFIG_IOCACHE
struct work_struct pwrkey_long_press_iocache_work;
#endif
...
#ifdef CONFIG_HW_BFMR_MTK
#ifdef CONFIG_IOCACHE
static void pwrkey_long_press_iocache_work_func(struct work_struct *work)
{
	unsigned long timeout = jiffies + 4 * HZ;
	printk(KERN_ERR "pwrkey_long_press_iocache_work_func\n");

	while (1) {
		if (time_after(jiffies, timeout)) {
			printk(KERN_ERR "iocache flush timout 4 second");
			break;
		}
		iocache_drains_notify(NULL, 0, NULL);
		msleep(5);
	}
	return;
}
...
#ifdef CONFIG_IOCACHE
static void pwrkey_iocache_long_press(unsigned long pressed)
{
	printk(KERN_ERR "enter pwrkey_iocache_long_press,init=%d\n", pwrkey_long_press_work_init);
	if (pwrkey_long_press_work_init)
		schedule_work(&pwrkey_long_press_iocache_work);
}
#endif
...
#endif
#ifdef CONFIG_IOCACHE
static void hw_iocache_long_press(unsigned long pressed)
{

	if (!pwrkey_long_press_timer_init) {
		pr_err(" iocache pwrkey long press timer is not init!\n");
		return;
	}
	if (pressed && (!timer_pending(&long_press_pwrkey_timer))) {
		long_press_pwrkey_timer.expires = jiffies + 5*HZ;
		...
		long_press_pwrkey_timer.function =
			pwrkey_iocache_long_press;
		add_timer(&long_press_pwrkey_timer);
		pr_err("iocache pwrkey_long_press add_timer\n");
	} else {
		pr_err("iocache pwrkey_long_press del_timer_sync\n");
		del_timer_sync(&long_press_pwrkey_timer);
	}
}
#endif
...
#ifdef CONFIG_IOCACHE
	INIT_WORK(&pwrkey_long_press_iocache_work,pwrkey_long_press_iocache_work_func);
#endif
