	...
	unsigned *repeat_pages, int *page_count_delta)
{
	...
	struct s_pagecache_info *myentry = NULL;
	struct s_pagecache_info *merged_entry = NULL;
...
static void workingset_collector_reset(struct s_workingset *ws)
{
	...
	g_selfread_state = E_SELFREAD_NONE;
...
static void workingset_mark_target_process(int pid, bool clear)
{
	...
	if (!clear) {
		spin_lock(&g_collector->lock);
		if (g_selfread_state != E_SELFREAD_INIT) {
			spin_unlock(&g_collector->lock);
			return;
		}
		g_selfread_state = E_SELFREAD_WAIT;
		spin_unlock(&g_collector->lock);
	}
...
{
	spin_lock(&g_collector->lock);
	if (g_selfread_state != E_SELFREAD_NONE) {
		spin_unlock(&g_collector->lock);
		return;
	}
	g_selfread_state = E_SELFREAD_INIT;
	spin_unlock(&g_collector->lock);
	...
	unsigned int *len, unsigned int *hashcode)
{
	...
	if (path_is_under_dirs(filepath, gExcludedDir))
		return -1;
...
{
	bool is_self_dir = path_is_under_dirs(path, g_selfread_dir);

	if ((g_selfread_state != E_SELFREAD_DOING) && is_self_dir)
		return false;
	else if ((g_selfread_state == E_SELFREAD_DOING) && !is_self_dir)
		return false;
	else
		return true;
}
...
try_next:
	...
	if (g_selfread_state != E_SELFREAD_DOING)
		check_cachemiss_threshold_rcrdlocked(record,
			present_pages_cnt, read_pages_cnt);
...
static void workingset_preread_by_self_internal(struct s_ws_record *record)
{
	struct task_struct *tsk = NULL;
	struct task_struct *leader = NULL;
...
void workingset_preread_by_self(void)
{
	...
	spin_lock(&g_collector->lock);
	if (g_selfread_state != E_SELFREAD_WAIT)
		goto out;
	...
	g_selfread_state = E_SELFREAD_DOING;
	spin_unlock(&g_collector->lock);
	record = workingset_get_existed_record_wslocked(&ws->owner, true);
	mutex_unlock(&ws->mutex);
